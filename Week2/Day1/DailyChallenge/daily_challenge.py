...
# of all the animal types (names) in the farm. With the example above, the list should be: ['cow', 'goat', 'sheep'].

# Add another method to the Farm class called get_short_info. This method should return the following string:
# “McDonald’s farm has cows, goats and sheeps.”. The method should call the get_animal_types function to get a list of the animals.
...
class Farm:
    ...
    def get_info(self):
        ...
        max_name_length = max(len(animal) for animal in self.animals.keys())

        for animal, count in sorted(self.animals.items()):
            info += f"{animal.ljust(max_name_length)} : {count}\n"
    ...
    def get_short_info(self):
        ...
        plural_animal_types = [
            animal + "s" if animal != "sheep" else animal for animal in animal_types
        ]
        animal_description = ", ".join(plural_animal_types)
        return f"{self.name}'s farm has {animal_description}."
